...
const Aegis128X2Raf = aegis_raf.Aegis128X2Raf(aegis_raf.MemoryStorage);
...
test "interop: libaegis creates AEGIS-128X2 RAF, this port reads it" {
    try testing.expectEqual(0, aegis_c.aegis_init());

    var key: [16]u8 = undefined;
    random.bytes(&key);
    // Exercise chunk authentication beyond index zero and a partial tail.
    const plaintext = try randomPayload(testing.allocator, 3 * 16384 + 1000);
    defer testing.allocator.free(plaintext);

    var c_file = CFile{ .allocator = testing.allocator };
    defer c_file.deinit();

    var scratch_buf: [aegis_c.AEGIS128X2_RAF_SCRATCH_SIZE(16384)]u8 align(aegis_c.AEGIS_RAF_SCRATCH_ALIGN) = undefined;
    const scratch = aegis_c.aegis_raf_scratch{ .buf = &scratch_buf, .len = scratch_buf.len };
    const cfg = aegis_c.aegis_raf_config{ .chunk_size = 16384, .flags = aegis_c.AEGIS_RAF_CREATE, .scratch = &scratch };

    var c_ctx: aegis_c.aegis128x2_raf_ctx align(32) = undefined;
    try testing.expectEqual(0, aegis_c.aegis128x2_raf_create(&c_ctx, &c_file.io(), &cRng(), &cfg, &key));

    var bytes_written: usize = undefined;
    try testing.expectEqual(0, aegis_c.aegis128x2_raf_write(&c_ctx, &bytes_written, plaintext.ptr, plaintext.len, 0));
    aegis_c.aegis128x2_raf_close(&c_ctx);

    var mem = aegis_raf.MemoryStorage.init(testing.allocator);
    defer mem.deinit();
    try mem.bytes.appendSlice(testing.allocator, c_file.data.items);

    var zig_ctx = try Aegis128X2Raf.open(testing.allocator, &mem, random, .{}, &key);
    defer zig_ctx.close();
...
test "interop: this port creates AEGIS-128X2 RAF, libaegis reads it" {
    try testing.expectEqual(0, aegis_c.aegis_init());
    ...
    {
        var zig_ctx = try Aegis128X2Raf.create(testing.allocator, &mem, random, .{ .chunk_size = 16384 }, &key);
        defer zig_ctx.close();
        _ = try zig_ctx.write(plaintext, 0);
        // Check interoperability after an overwrite and a shrink.
        _ = try zig_ctx.write(plaintext[0..100], 16384 - 50);
        try zig_ctx.setLength(2 * 16384 + 7);
    }
    const expected_len = 2 * 16384 + 7;
    ...
    var c_ctx: aegis_c.aegis128x2_raf_ctx align(32) = undefined;
    try testing.expectEqual(0, aegis_c.aegis128x2_raf_open(&c_ctx, &c_file.io(), &cRng(), &open_cfg, &key));
    defer aegis_c.aegis128x2_raf_close(&c_ctx);

    var size: u64 = undefined;
    try testing.expectEqual(0, aegis_c.aegis128x2_raf_get_size(&c_ctx, &size));
    try testing.expectEqual(expected_len, size);

    const buf = try testing.allocator.alloc(u8, expected_len);
    defer testing.allocator.free(buf);
    var bytes_read: usize = undefined;
    try testing.expectEqual(0, aegis_c.aegis128x2_raf_read(&c_ctx, buf.ptr, &bytes_read, expected_len, 0));
    try testing.expectEqual(expected_len, bytes_read);
    @memcpy(plaintext[16384 - 50 ..][0..100], plaintext[0..100]);
    try testing.expectEqualSlices(u8, plaintext[0..expected_len], buf);
}
